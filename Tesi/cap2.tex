...
In questo capitolo analizzeremo i protocolli che oggi rappresentano lo standard de facto nella condivisione dei file. Anche se molti sono conosciuti come programmi usati per scambio di materiale multimediale, i protocolli che vi sono alla base di tutti i programmi presenti sul mercato usati dai professionisti per condividere file. 
...
In particolare si vedranno le caratteristiche di Napster, Gnutella, Kademlia e Freenet\cite{freenetsite}.
Nell'analizzare queste reti però teniamo sempre presente il nostro caso di studio, il gruppo di professionisti che deve condividere materiale sensibile, e presteremo infatti attenzione a quelli che possono essere i punti a noi favorevoli\cite{slashdot} e quelli che invece andrebbero a discapito dei nostri obbiettivi.
\subsection{Napster}
\begin{figure}
\begin{center}
...
E' il primo protocollo che analizziamo che sfrutta la tecnologia delle DHT ovvero Distributed Hash Tables. La tecnica delle DHT nasce dalla collaborazione di alcune comunità di file sharing e viene subito adottata in diversi software\cite{kademlia}.
...
Il programma che abbiamo scelto di studiare nella categoria darknet è stato Freenet\cite{freenet}. Questa scelta è stata dettata dalla grande comunità che gli sta alle spalle, dal fatto che sia disponibile il codice sorgente, e dalle caratteristiche che più si avvicinavano al nostro modello. Molto utile in questo senso è stato lo studio della pubblicazione 'Beyond Simulation: LArge-Scale Distribuited Emulation of P2P Protocols'\cite{beyond}, prodotta da due ricercatori tedeschi, nella quale si evidenza la forte differenza di scalabilità a seconda del tipo di topologia di una rete. Infatti Freenet è una rete decentralizzata, cioè dove non abbiamo server ma tutte le informazioni sono distribuite sui nodi. E' una rete realizzata puntando ad un alto livello di sicurezza ed anonimato. 
...
I servizi che vengono offerti nel campo del cloud computing si possono dividere in tre tipologie. I servizi SaaS ovvero Sofware as a Service, PaaS Platform as a Service o IaaS Infrastructure as a Service. Questi diversi servizi diversificano il tipo di servizio offerto, il servizio IaaS per esempio permettel'utilizzo di risorse hardware in remoto, servizio simile al Grid Computing ma con una caratteristica imprescindibile: le risorse vengono utilizzate su richiesta o domanda al momento in cui una piattaforma ne ha bisogno, non vengono assegnate a prescindere dal loro utilizzo effettivo. Tramite un servizio IaaS per esempio possiamo avere un diversi sistemi operativi attivi su server remoti, o ancora è di questo tipo il servizio di storage online nel quale ci viene dato spazio di archiviazione online. I servizi PaaS permettono invece di eseguire in remoto una piattaforma software formata da diversi programmi. Infine i servizi SaaS consentono l'utilizzo in remoto di singoli programmi. I programmi utilizzabili su SaaS però devono essere realizzati ottimizzati per il servizio, infatti ogni provider di servizi cloud ha una propria serie di librerie, che contengono i comandi ottimizzati per le loro infrastrutture. Questo offre il vantaggio di avere software molto veloce ed efficiente, essendo ottimizzato, d'altra parte però ci obbliga a sviluppare software ad hoc per ogni diverso servizio di cloud computing. Per esempio le API del servizio cloud di Google sono diverse da quelle di Amazon che sono ancora diverse di Microsoft. Noi vogliamo realizzare Darkcloud in modo che sia facile in un secondo momento poterlo adattare a diversi servizi di cloud, e questo sarà possibile utilizzando molto l'incapsulamento e la modularità nel codice in modo da renderlo facilmente riutilizzabile.
